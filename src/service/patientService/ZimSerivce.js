...
const appConfig = {
    appID: AppID, // Use the AppID from utils
    serverSecret: ServerSecret, // Use the ServerSecret from utils
};
...
export function generateToken(userID, seconds) {
  ...
  console.log("Token generation details:", {
    ...
    serverSecret: appConfig.serverSecret ? "***" : "Missing", // Don't log actual secret
    time,
    expire: body.expire
  });
  ...
  console.log('generateToken completed, token length:', token.length);
